...
test('AI lab has four distinct child-safety learning scenarios', () => {
  assert.equal(lab.scenarios.length, 4);
...
test('summary requires three correct answers and does not claim mastery', () => {
  assert.equal(lab.summarize([true, true, false, false]).passed, false);
  const passing = lab.summarize([true, true, true, false]);
  assert.equal(passing.passed, true);
  assert.equal(passing.correct, 3);
