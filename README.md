...
## הפעלה מקומית
פותחים את `index.html` בדפדפן. זהו. כל הדפים טוענים את אותה ליבה (`src/lib/core.js` + המודולים שהוא צריך) — בלי CDN ובלי bundler.
...
אין בדיקת WCAG מול NVDA/VoiceOver בכיתה אמיתית. ה-GitHub Actions היחיד בריפו מריץ את חבילת הבדיקות בלבד (`.github/workflows/validate.yml`) — לא ממזג, לא דוחף בכוח ולא פורס.
